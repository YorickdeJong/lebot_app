...
const getRobot = async (req, res) => {
    ...
    try {
        ...
        if(rows.length === 0){
            res.status(404).send('No robots found');
        }
...
const getRobotByGroupIdClassId = async (req, res) => {
    ...
    try {
        ...
        if(rows.length === 0) {
            res.status(200).json({});  // send an empty object back to the client
            return;
        }
    ...
    try {
        for(let i = 0; i < numRobots; i++) {
            ...
            if(result.rows.length === 0) {
                res.status(500).json({message: 'Error creating robots'});
                return;
            }
...
const updateRobot = async (req, res) => {
    ...
    try {
        ...
        if(result.rows.length === 0) {
            res.status(404).json({message: 'Robot not found'}); // send a not found message back to the client
            return;
        }
...
const deleteRobotGroup = async (req, res) => {
    ...
    try {
        ...
        if(result.rows.length === 0) {
            res.status(200).json({message: 'No robots to delete'}); // send a success message back to the client
            return;
        }
